...
class ExpensesController {
    ...
    try {
      ...
      if (paymentName.name === `наличка`) {
        await Balance.update(
          {
            cash: Number(balance[0].cash) - Number(sum),
            totalCash: Number(balance[0].totalCash) - Number(sum),
          },
      ...
      if (paymentName.name === `безнал`) {
        await Balance.update(
          {
            cashless: Number(balance[0].cashless) - Number(sum),
            totalCash: Number(balance[0].totalCash) - Number(sum),
          },
          { where: { id: balance[0].id } }
        );
    ...
    try {
      ...
      const arrayResult = {
        ...
        rows: data.rows.map((expense) => ({
          ...
          paymentName: expense.paymentsName
            ? expense.paymentsName.name
            : "Без типа оплаты",
        })),
      };
  ...
  async edit(req, res, next) {
    try {
      ...
      if (
        paymentId === `0` &&
        payment.name === `наличка` &&
        Number(sum) !== Number(expenses.sum)
      ) {
        const difference = Number(sum) - Number(expenses.sum);

        await Balance.update(
          {
            cash: Number(balance[0].cash) - difference,
            totalCash: Number(balance[0].totalCash) - difference,
          },
          { where: { id: balance[0].id } }
        );
      }
      ...
      if (
        ...
        await Balance.update(
          {
            cashless: Number(balance[0].cashless) - difference,
            totalCash: Number(balance[0].totalCash) - difference,
          },
          { where: { id: balance[0].id } }
        );
      ...
      if (
        ...
        newPayment.name === `наличка`
      ) {
        const difference = Number(sum) - Number(expenses.sum);

        await Balance.update(
          {
            cash: Number(balance[0].cash) + Number(expenses.sum) - difference,
            cashless: Number(balance[0].cashless) - Number(expenses.sum),
            totalCash: Number(balance[0].totalCash) - difference,
          },
          { where: { id: balance[0].id } }
        );
      ...
      if (
        paymentId !== `0` &&
        payment.id !== paymentId &&
        newPayment.name === `безнал`
      ) {
        const difference = Number(sum) - Number(expenses.sum);

        await Balance.update(
          {
            cash: Number(balance[0].cash) - Number(expenses.sum),
            cashless:
              Number(balance[0].cashless) + Number(expenses.sum) - difference,
            totalCash: Number(balance[0].totalCash) - difference,
          },
          { where: { id: balance[0].id } }
        );
